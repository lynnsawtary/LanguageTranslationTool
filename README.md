# Language Translation Tool

A modern Language Translation Tool developed using Python and Tkinter.

The application allows users to translate text between multiple languages using AI-powered translation services. It includes text-to-speech functionality, clipboard copying, and a clean graphical user interface.

---

# Features

- Translate text between multiple languages
- Auto language detection
- User-friendly graphical interface
- Copy translated text to clipboard
- Text-to-speech support
- Error handling and validation
- Modern dark-themed UI

---

# Technologies Used

- Python
- Tkinter
- deep-translator
- pyttsx3
- pyperclip

---

# Project Structure

```text
LanguageTranslationTool/
│
├── main.py
├── README.md
├── requirements.txt
├── .gitignore
└── venv/
```

---

# Installation

Clone the repository:

```bash
git clone https://github.com/YOUR_USERNAME/LanguageTranslationTool.git
```

Navigate into the project folder:

```bash
cd LanguageTranslationTool
```

Create a virtual environment:

```bash
python -m venv venv
```

Activate the virtual environment:

### Windows

```bash
venv\Scripts\activate
```

Install dependencies:

```bash
pip install -r requirements.txt
```

---

# Usage

Run the application:

```bash
python main.py
```

Enter text, select source and target languages, then click:

```text
Translate
```

Additional features:

- Copy translated text
- Listen to translated text using speech
- Clear input/output fields

---

# How It Works

1. The user enters text into the input field.
2. The application sends the text to the translation service.
3. The translated response is received and displayed.
4. Users can copy or listen to the translated result.


---

# Example Applications

- Language learning
- Travel communication
- Educational tools
- Accessibility support
- Multilingual communication

---

# Future Improvements

Possible future enhancements:

- Voice input support
- Translation history
- File translation support
- OCR image translation
- Theme customization
- Web-based deployment

---

# Author

Lynn Sawtary

